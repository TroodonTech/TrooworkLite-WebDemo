...
app.options('/addtaskSchedule', supportCrossOriginScript);
app.post(securedpath + '/addtaskSchedule', supportCrossOriginScript, function (req, res) {

    var newWOObj = {};
    newWOObj = req.body;
    var scheduleKey = newWOObj.taskName;
    var workorderkey = newWOObj.workorderkey;
    var workordertypekey = newWOObj.workordertypekey;
    var equipmentkey = newWOObj.equipmentkey;
    var roomkeys = newWOObj.roomkeys;
...
app.get(securedpath + '/viewScheduledTasks', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    var empkey = url.parse(req.url, true).query['employeekey'];
    var workDT = url.parse(req.url, true).query['viewdate'];
    var pageno = url.parse(req.url, true).query['pageno'];
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query("set @employeekey =?;set @workdate =?;set @pageno=?; set @itemsPerPage=?;set @OrganizationID=?;call usp_task_viewScheduledTasks(@employeekey,@workdate,@pageno,@itemsPerPage,@OrganizationID)", [empkey, workDT, pageno, itemsPerPage, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.post(securedpath + '/taskScheduleByallFilters', supportCrossOriginScript, function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query("set @manager =?;set @workorderStatusKey =?;set @workorderDate =?;set @workorderDate2 =?;set @facilitykey=?; set @roomTypeKey=?;set @floorKey=?;set @roomKey=?;set @zoneKey=?;set @employeekey=?;set @workorderTypeKey=?;set @batchScheduleNameKey=?;set @OrganizationID=?;call usp_task_taskScheduleByallFilters(@manager,@workorderStatusKey,@workorderDate,@workorderDate2,@facilitykey,@roomTypeKey,@floorKey,@roomKey,@zoneKey,@employeekey,@workorderTypeKey,@batchScheduleNameKey,@OrganizationID)", [manager, workorderStatusKey, workorderDate, workorderDate2, facilitykey, roomTypeKey, floorKey, roomKey, zoneKey, employeekey, workorderTypeKey, batchScheduleNameKey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.post(securedpath + '/searchTaskScheduleByallFilters', supportCrossOriginScript, function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query("set @manager =?;set @workorderStatusKey =?;set @workorderDate =?;set @workorderDate2 =?;set @facilitykey=?; set @roomTypeKey=?;set @floorKey=?;set @roomKey=?;set @zoneKey=?;set @employeekey=?;set @workorderTypeKey=?;set @batchScheduleNameKey=?;set @OrganizationID=?; set@searchWO=?; call usp_task_searchTaskScheduleByallFilters(@manager,@workorderStatusKey,@workorderDate,@workorderDate2,@facilitykey,@roomTypeKey,@floorKey,@roomKey,@zoneKey,@employeekey,@workorderTypeKey,@batchScheduleNameKey,@OrganizationID,@searchWO)", [manager, workorderStatusKey, workorderDate, workorderDate2, facilitykey, roomTypeKey, floorKey, roomKey, zoneKey, employeekey, workorderTypeKey, batchScheduleNameKey, OrganizationID, searchWO], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.post(securedpath + '/deletebatchTasks', function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            connection.query('set @deletebatchWorkOrderString=?; set @employeekey=?;set @OrganizationID=?;  call usp_task_deletebatchTasks(@deletebatchWorkOrderString,@employeekey,@OrganizationID)', [deletebatchWorkOrderString, employeekey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.get(securedpath + '/getRemainingTaskDetails', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    var from = url.parse(req.url, true).query['from'];
    var to = url.parse(req.url, true).query['to'];
    var empKey = url.parse(req.url, true).query['empKey'];
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query('set @from=?;set @to=?;set @empkey=?;set @wotypeKey=?;set @org=?; call usp_task_getRemainingTaskDetails(@from,@to,@empkey,@wotypeKey,@org)', [from, to, empKey, wotypeKey, org], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.post(securedpath + '/taskReportByallFilters', supportCrossOriginScript, function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query("set @manager =?;set @workorderStatusKey =?;set @workorderDate =?;set @workorderDate2 =?;set @facilitykey=?; set @roomTypeKey=?;set @floorKey=?;set @roomKey=?;set @zoneKey=?;set @employeekey=?;set @OrganizationID=?; set@WorkorderTypeKey=?;call usp_task_taskReportByallFilters(@manager,@workorderStatusKey,@workorderDate,@workorderDate2,@facilitykey,@roomTypeKey,@floorKey,@roomKey,@zoneKey,@employeekey,@OrganizationID,@WorkorderTypeKey)", [manager, workorderStatusKey, workorderDate, workorderDate2, facilitykey, roomTypeKey, floorKey, roomKey, zoneKey, employeekey, OrganizationID, WorkorderTypeKey], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.post(securedpath + '/getEmployeeForPieTASK', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    var newWOObj = {};
    newWOObj = req.body;
    var date = newWOObj.Date;
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query('set @date=?; set @date1=?; set @empkey=?; set @managerKey=?; set @WorkorderTypeKey=?;set @OrganizationID=?;  call usp_task_getEmpvaluesforpie(@date,@date1,@empkey,@managerKey,@WorkorderTypeKey,@OrganizationID)', [date, date1, empkey, managerKey, WorkorderTypeKey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.post(securedpath + '/TASKByfilterPie', supportCrossOriginScript, function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query("set @manager =?; set @workorderDate =?; set @workorderDate2 =?; set @employeekey=?; set @workorderTypeKey=?;set @OrganizationID=?;  call usp_task_taskByallFiltersPie(@manager,@workorderDate,@workorderDate2,@employeekey,@workorderTypeKey,@OrganizationID)", [manager, workorderDate, workorderDate2, employeekey, workorderTypeKey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.post(securedpath + '/deleteTaskBatchSchedule', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    var workschedulekey = req.body.workorderSchedulekey;
    var OrganizationID = req.body.OrganizationID;
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            connection.query('set @workschedulekey=?;set @OrganizationID=?;  call usp_task_deleteTaskBatchSchedule(@workschedulekey,@OrganizationID)', [workschedulekey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
                    // console.log("deleteTaskBatchSchedule...from server.." + JSON.stringify(rows[2]));
                    res.end(JSON.stringify(rows[2]));
                }
            });
        }
...
app.options('/taskViewSupervisorByAll', supportCrossOriginScript);
app.post(securedpath + '/taskViewSupervisorByAll', supportCrossOriginScript, function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query('set @OrganizationID=?;set @employeekey=?;set @startDate=?;set @endDate=?;set @SearchWO=?; set@RoomTypeKey=?; set@FloorKey=?; set@ZoneKey=?; set@FacilityKey=?; set@pageNo=?;set@schedulename=?;set@employee=?; set@itemsPerPage=?; set@isFiltered=?; call usp_task_taskViewSupervisorByAll(@OrganizationID,@employeekey,@startDate,@endDate,@SearchWO,@RoomTypeKey,@FloorKey,@ZoneKey,@FacilityKey,@pageNo,@schedulename,@employee,@itemsPerPage,@isFiltered)', [OrganizationID, employeekey, startDate, endDate, SearchWO, RoomTypeKey, FloorKey, ZoneKey, FacilityKey, pageNo, schedulename, employee, itemsPerPage, isFiltered], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.post(securedpath + '/taskEmployeeByallFilters', supportCrossOriginScript, function (req, res) {
    var newWOObj = {};
    newWOObj = req.body;
    var manager = newWOObj.manager;
    var workorderDate = newWOObj.workorderDate;
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query("set @manager =?;set @workorderDate =?;set @workorderDate2 =?;set @facilitykey=?; set @roomTypeKey=?;set @floorKey=?;set @zoneKey=?;set @OrganizationID =?;call usp_task_taskEmployeeByallFilters(@manager,@workorderDate,@workorderDate2,@facilitykey,@roomTypeKey,@floorKey,@zoneKey,@OrganizationID)", [manager, workorderDate, workorderDate2, facilitykey, roomTypeKey, floorKey, zoneKey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.get(securedpath + '/barcodeRoom_Task', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    var barcode = url.parse(req.url, true).query['barcode'];
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query("set @workdetail =?;set @barcode=?; set @empkey=?; set @updatetype=?; set @OrganizationID=?;set @complete_Time=?;call usp_task_TaskStatusUpdateByBarcode(@workdetail,@barcode,@empkey,@updatetype,@OrganizationID,@complete_Time)", [workorderkey, barcode, employeekey, updatetype, OrganizationID, complete_Time], function (err, rows) {
                if (err) {
                    console.log(err);
                }
                else {
...
app.get(securedpath + '/updateTaskByPhoto', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    var pho = url.parse(req.url, true).query['pho'];
    var wdkey = url.parse(req.url, true).query['wkey'];
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query(" set @wdk=?;set @imgname=?; set @employeekey=?; set @OrganizationID=?;set @complete_Time=?;call usp_task_TaskStatusUpdateByPhoto(@wdk,@imgname,@employeekey,@OrganizationID,@complete_Time)", [wdkey, pho, employeekey, OrganizationID, complete_Time], function (err, rows) {
                if (err) {
                    console.log(err);
                }
                else {
...
app.get(securedpath + '/TaskCompleted', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    var employeekey = url.parse(req.url, true).query['employeekey'];
    var work_det_key = url.parse(req.url, true).query['wkey'];
    var OrganizationID = url.parse(req.url, true).query['OrganizationID'];
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query("set @workdetail =?; set @employeekey=?; set @OrganizationID=?;call usp_task_TaskStatusUpdate(@workdetail,@employeekey,@OrganizationID)", [work_det_key, employeekey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log(err);
                }
                else {
...
app.options('/cancelTask', supportCrossOriginScript);
app.post(securedpath + '/cancelWorkOrder_SuType', supportCrossOriginScript, function (request, res) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query('set @workOrderKey=?;set @reason=?;set @updateDate=?;set @updateTime=?; set @empKey=?; set @OrganizationID=?; call usp_task_TaskCancel(@workOrderKey,@reason,@updateDate,@updateTime,@empKey,@OrganizationID)', [workOrderKey, reason, updateDate, updateTime, empKey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                } else {
                    res.end(JSON.stringify(rows[4]));
...
app.post(securedpath + '/deleteTasks', function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            connection.query('set @deleteWorkOrderString=?; set @employeekey=?;set @OrganizationID=?;  call usp_task_Tasksdelete(@deleteWorkOrderString,@employeekey,@OrganizationID)', [deleteWorkOrderString, employeekey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.options('/taskViewsEmpByAll', supportCrossOriginScript);
app.post(securedpath + '/taskViewsEmpByAll', supportCrossOriginScript, function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query('set @OrganizationID=?;set @employeekey=?;set @startDate=?;set @endDate=?;set @SearchWO=?; set@RoomTypeKey=?; set@FloorKey=?; set@ZoneKey=?; set@FacilityKey=?; set@pageNo=?; set@itemsPerPage=?; set@isFiltered=?; call usp_task_taskViewsEmpByAll(@OrganizationID,@employeekey,@startDate,@endDate,@SearchWO,@RoomTypeKey,@FloorKey,@ZoneKey,@FacilityKey,@pageNo,@itemsPerPage,@isFiltered)', [OrganizationID, employeekey, startDate, endDate, SearchWO, RoomTypeKey, FloorKey, ZoneKey, FacilityKey, pageNo, itemsPerPage, isFiltered], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.get(securedpath + '/checkTaskName', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query("set @taskName=?; set @OrganizationID=?;call usp_task_checkTaskName(@taskName,@OrganizationID)", [taskName, OrganizationID], function (err, rows) {
                if (err) {
                    console.log(err);
                }
                else {
...
app.get(securedpath + '/getTaskNameList', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query(' set @employeekey=?;set @OrganizationID=?; call usp_getTaskNameList(@employeekey,@OrganizationID)', [employeekey, OrganizationID], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
...
app.get(securedpath + '/getCompletedTaskDetails', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    var from = url.parse(req.url, true).query['from'];
    var to = url.parse(req.url, true).query['to'];
    var empKey = url.parse(req.url, true).query['empKey'];
    ...
    pool.getConnection(function (err, connection) {
        if (err) {
            ...
            console.log("Success! Connection with Database spicnspan via connection pool succeeded");
            connection.query('set @from=?;set @to=?;set @empkey=?;set @wotypeKey=?;set @org=?; call usp_task_getCompletedTaskDetails(@from,@to,@empkey,@wotypeKey,@org)', [from, to, empKey, wotypeKey, org], function (err, rows) {
                if (err) {
                    console.log("Problem with MySQL" + err);
                }
                else {
