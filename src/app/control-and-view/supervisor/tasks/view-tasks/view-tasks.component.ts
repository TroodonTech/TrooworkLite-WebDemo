...
export class ViewTasksComponent implements OnInit {
  ...
  constructor(private WorkOrderServiceService: WorkOrderServiceService, private formBuilder: FormBuilder, private el: ElementRef, private taskServ: TaskService) { }
  ...
  previousPage() {
    ...
    this.loading = true;
    this.taskServ.taskViewsSupervisorByAll(Wo)
      .subscribe((data: any[]) => {
  ...
  viewEmployeeWorkorderByFilter() {
    ...
    // else{
    this.taskServ
      .gettaskTablewithOnDateandToDateFilter(date1, date2, this.toServeremployeekey, this.OrganizationID, this.FacilityKey, this.FloorKey, this.RoomTypeKey, this.ZoneKey)
      .subscribe((data: any[]) => {
        this.WorkorderDetTable = data;
        this.loading = false;
  ...
  workorderCompleted(i, barcodeRequired, photoRequired, workorderkey, file) {
    ...
    if (!(this.fileName) && photoRequired === 1) {
      ...
      this.taskServ
        .UpdateTaskbyPhotoForEmployee(this.fileName, this.toServeremployeekey, workorderkey, this.OrganizationID, p)
        .subscribe((data: any[]) => {
        });
    }
    if (photoRequired !== 1 && barcodeRequired !== 1) {
      this.taskServ
        .CompleteTaskByempWithoutPhotoandBarcd(this.toServeremployeekey, workorderkey, this.OrganizationID, p)
        .subscribe((data: any[]) => {
          this.FinishButton[i] = true;
        });
    }
  ...
  taskViewsEmpByAll() {
    ...
    this.loading = true;
    this.taskServ.taskViewsSupervisorByAll(Wo).subscribe((data: any[]) => {
      this.WorkorderDetTable = data;
      this.loading = false;// loading
      if (data.length > 0) {
        if (this.WorkorderDetTable[0].totalItems > this.itemsPerPage) {
  ...
  canceltheWorkorder(woKey) {
    ...
    if ((reason.trim())) {
      this.taskServ
        .setCancelTask(woKey, reason, today_DT, p, this.toServeremployeekey, this.OrganizationID)
        .subscribe((data: any[]) => {
