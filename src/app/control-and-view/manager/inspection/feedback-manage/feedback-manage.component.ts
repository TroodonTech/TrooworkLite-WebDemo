...
export class FeedbackManageComponent implements OnInit {
  ...
  savetemplate() {
    ...
    if (count1 > 0) {
      ...
      this.message = "Don't leave question area empty";
    }
    else {
      if (this.tempID == 0) {
        this.inspectionService.createMasterReviewTempalte(this.toServeremployeekey, this.OrganizationID).subscribe((data: any[]) => {
          ...
          for (var j = 0; j < temp_insertArry.length; j++) {
            ...
            this.inspectionService
              .insertFeedbackQuestion(this.insertObj).subscribe((data: any[]) => { count = j + 1; });
          }
        });
      } else {
        count = 0;
  ...
  alertme() {
    ...
    this.message = "Questions added successfully";
    setTimeout(() => {
      this.newAttribute = [];
      this.loading = true;
      this.inspectionService
