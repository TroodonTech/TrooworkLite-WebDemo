...
@Component({
  selector: 'app-view-task-service-request',
  templateUrl: './view-task-service-request.component.html',
  styleUrls: ['./view-task-service-request.component.scss']
})
export class ViewTaskServiceRequestComponent implements OnInit {
  ...
  constructor(private WorkOrderServiceService: WorkOrderServiceService, private SchedulingService: SchedulingService, private task: TaskService) { }
      ...
      .subscribe((data: any[]) => {
        ...
        if (this.requestdetails.length > 0) {
          for (var i = 0; i < this.requestdetails.length; i++) {
            if (!this.requestdetails[i].EmployeeKey) {
              this.requestdetails[i].EmployeeKey = "";
            }
          }
        }
      });
  ...
  createtaskbyservicerequest(servicerequestid, empKey) {
    ...
    this.task.generatetaskbyservicerequest(this.vpto)
      .subscribe((data: any[]) => {
        this.requestdetails = data;
        if (data.length > 0) {
